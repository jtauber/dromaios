...
RAM and ports are untouched. The core still guards the execution boundary and
records detached before/after snapshots around this action.
...
Fetching, instruction retirement, and interrupt entry remain in the runtime.
The address-stack and flag sections below explain how instructions use the
stored fields.
...
encodings (`22`, `2A`, `32`, `38`, `39`, and `3A`) stay absent. Opcode fetching,
boundary guarding and records, and interrupt-supplied execution remain in the
CPU core; this chapter owns stored state, views, reset effects, and instructions.
