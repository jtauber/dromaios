...
of the microcomputer era. This guide follows it from its origins to its
...
<diagram padding="24" theme="light" background="#fffefa">
  <row gap="70" align="center">
    <contains label="8008" layout="graph" direction="lr" gap="40" row-gap="50" padding="18">
      <rect id="registers" width="140" height="70">A B C D E H L</rect>
      <rect id="alu" width="140" height="70">8-bit arithmetic and logic</rect>
      <rect id="flags" width="140" height="70">S Z P C flags</rect>
      <rect id="hl" width="140" height="70">H:L data address</rect>
      <rect id="addresses" width="140" height="70">Eight 14-bit address registers</rect>
      <rect id="pc" width="140" height="70">Selected PC</rect>
      <rect id="control" width="140" height="70">Instruction decoding and control</rect>
    </contains>
    <column gap="60">
      <rect id="memory" width="140" height="70">External memory: code and data</rect>
      <rect id="ports" width="140" height="70">External input and output ports</rect>
    </column>
  </row>
  <connect from="registers" to="alu" arrow="both" moves="false"/>
  <connect from="alu" to="flags"/>
  <connect from="registers" to="hl"/>
  <connect from="addresses" to="pc"/>
  <connect from="control" to="registers"/>
  <connect from="control" to="addresses"/>
  <connect from="control" to="alu"/>
  <connect from="pc" to="memory"/>
  <connect from="hl" to="memory"/>
  <connect from="memory" to="control"/>
  <connect from="memory" to="registers" arrow="both"/>
  <connect from="registers" to="ports" arrow="both"/>
</diagram>
```
...
This specification defines every documented instruction and the externally visible
...
specification is the sole processor-specific implementation source.
...
The build discovers this specification and registers the model from `cpu "8008"`.
...
The specification calls these locations ADDRESS and SELECTOR. They refer to the
