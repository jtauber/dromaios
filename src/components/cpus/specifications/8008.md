...
transfers, arithmetic, logic, adjustments, rotates, control flow, and port I/O. The same definitions generate
...
existing stored fields `addressStack` and `stackIndex`; SELECTOR is an internal
selection register, not an additional software-visible byte register. The
schema remains responsible for storage and public state types.
