...
loads and stores, transfers, logic, comparisons, BIT, arithmetic, and byte updates
...
rotation writes that change live carry. The existing
...
Stack/status instructions, control flow, NOP, and interrupt entry still use
TypeScript-authored definitions. The core still owns
