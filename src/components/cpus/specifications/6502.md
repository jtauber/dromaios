...
loads and stores, register transfers, logical operations, comparisons, and BIT
...
The remaining instruction families and execution boundaries still live in
TypeScript. The [coverage report](../../../../docs/cpus/coverage.md#6502)
tracks that migration.
...
bit 5 and stacked B marker are not extra state fields. Its remaining packing
rules, reset effects, and interrupt recognition still belong to the
[model contract](../../../../docs/cpus/6502/model.md). Changing a register does
...
ORA/AND/EOR/ADC/STA/LDA/CMP/SBC in binary order; ADC/SBC still consume this
chapter's addressing sources from their TypeScript definitions. The three `b`
...
operand ordering, and diagnostics. The existing
[CPU tests](../../../../tests/components/cpus/6502.test.ts) and machine tests
retain their independently specified results, wrapping, preservation rules,
access order, and failure behavior.

ADC/SBC, shifts and increments, stack/status instructions, control flow, and
interrupt entry still use TypeScript-authored definitions. The core still owns
...
the existing state adapter alongside its remaining packed-status layout.
