...
policy ADCDECIMAL "ADC NMOS decimal flags" (left: 8, right: 8, binary: 8, intermediate: 16) {
  ...
  C = not(lessThan(intermediate, u16($A0), unsigned))
}
```
...
family ADC "011 bbb 01" for b in accumulator.read {
  ...
  when decimal {
    ...
    correctedLow = select(not(lessThan(low, u16($0A), unsigned)), lowCorrection, low)
    ...
    correction = select(not(lessThan(intermediate, u16($A0), unsigned)), u16($60), u16($00))
