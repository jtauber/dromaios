...
This specification is the sole processor-specific implementation source for Dromaios's
...
specification follows the original NMOS model; later CMOS instruction additions and
...
<diagram padding="24" theme="light" background="#fffefa">
  <graph direction="lr" gap="28" row-gap="60">
    <rect id="a" width="160" height="66">A: accumulator</rect>
    <rect id="alu" width="160" height="66">Arithmetic and logic</rect>
    <rect id="flags" width="160" height="66">N V D I Z C</rect>
    <rect id="indices" width="160" height="66">X and Y</rect>
    <rect id="addressing" width="160" height="66">Address calculation</rect>
    <rect id="memory" width="160" height="66">64 KiB memory</rect>
    <rect id="pc" width="160" height="66">PC</rect>
    <rect id="sp" width="160" height="66">SP</rect>
    <rect id="stack" width="160" height="66">Page-one stack</rect>
  </graph>
  <connect from="a" to="alu" arrow="both"/>
  <connect from="flags" to="alu" arrow="none"/>
  <connect from="indices" to="addressing"/>
  <connect from="addressing" to="memory"/>
  <connect from="pc" to="memory"/>
  <connect from="sp" to="stack"/>
  <connect from="stack" to="memory"/>
</diagram>
```
...
bit 5 and stacked B marker are not extra state fields. The specification owns their
...
The [specification's stored-state declaration](#stored-state)
...
The ADC low digit is corrected when its word sum reaches ten; an unsigned
comparison expresses that threshold. Masking the corrected digit and adding
...
```cpu
policy ADCDECIMAL "ADC NMOS decimal flags" (left: 8, right: 8, binary: 8, intermediate: 16, decimalCarry: flag) {
  Z = zero(binary)
...
family ADC "011 bbb 01" for b in accumulator.read {
  ...
  when decimal {
    ...
    adjustLow: flag = not(lessThan(low, u16($0A), unsigned))
    correctedLow = select(adjustLow, lowCorrection, low)
    high = add(extend(and(left, u8($F0)), 16), extend(and(right, u8($F0)), 16))
    intermediate = add(high, correctedLow)
    adjustHigh: flag = not(lessThan(intermediate, u16($A0), unsigned))
    apply ADCDECIMAL(left, right, binary, intermediate, adjustHigh)
    correction = select(adjustHigh, u16($60), u16($00))
...
The same view supplies the specification's external interrupt entry action.
...
source indirectJump "NMOS page-wrapped pointer": 16 {
  ...
  low = memory(pointer)
  next = withBits(pointer, 7, 0, add(lowByte(pointer), u8($01)))
  high = memory(next)
  return concat(high, low)
}
```
...
selected steps show why addressing and flag effects stay explicit in the specification.
...
updates remain ordered effects; the specification does not infer flags by inspecting
the final register snapshot. The linked example specifications retain the
complete traces and stopping criteria.

## Checks and examples

[Specification tests](../../../../tests/components/cpus/semantics/literate.test.ts)
check the exact production opcode inventory, state schema, formal edits,
operand ordering, and diagnostics. [Arithmetic specification tests](../../../../tests/components/cpus/semantics/literate-6502-arithmetic.test.ts)
check edited decimal corrections, overflow predicates at three widths, and
rotation writes that change live carry. [Control/stack specification tests](../../../../tests/components/cpus/semantics/literate-6502-control.test.ts)
...
[Lifecycle specification tests](../../../../tests/components/cpus/semantics/literate-6502-lifecycle.test.ts)
...
Program images and acceptance traces remain in their example specifications.

## Reference comparisons

The earlier [applepy][applepy] and [dromaios-apple2][apple2] implementations
provided addressing, inspection, and teaching examples. These links pin the
versions examined; neither implementation establishes hardware correctness.
Their operation/addressing separation informed the current explicit operand
reads and writes. Their separate execution and [disassembly tables][apple2-disassembler] also motivate
deriving future disassembly from the executable definitions.

Decimal arithmetic needs independent evidence: applepy's ADC asserts that D is
clear, while dromaios-apple2's ADC ignores it. A recorded check of the latter
with A=`09`, operand=`01`, C=0, D=1 produced A=`0A`, leaving D set. The
[binary and decimal arithmetic contract](#arithmetic-in-binary-and-decimal)
instead uses NMOS correction and intermediate flags, with exhaustive local
checks and the independent references below.

Applepy's [addressing tests][applepy-tests] remain useful examples of wrapping
boundaries. Its [cycle notes][applepy-cycles] offer teaching ideas, not a verified
timing specification. Instruction-level records do not establish cycle totals
or complete bus traces. The [inspection example](../../../../docs/pedagogy.md#inspection-without-side-effects)
records a device-read pitfall from the Apple II explainer; labels and annotations
belong to lessons and instruments, as in its [instrument design][apple2-instruments].
...
[applepy]: https://github.com/jtauber/applepy/blob/934bf1a495583e7b4b08d42eae27e3532b51c3f2/cpu6502.py
[apple2]: https://github.com/jtauber/dromaios-apple2/blob/569baf98006f61e80ed93c36aa4f8d9ae23011d3/js/cpu.js
