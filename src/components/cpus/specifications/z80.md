...
It owns stored state, pair and status views and writes, and every unprefixed
instruction. The remaining prefixed instruction families and execution boundary
still use TypeScript. The
...
Prefixed instruction families and the execution lifecycle remain to migrate. Reusing an action for indexed arithmetic
