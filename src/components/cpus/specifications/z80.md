...
This is the executable instruction guide to Dromaios's instruction-level Z80.
It owns stored state, pair and status views and writes, all 698 documented
instruction forms, their prefix layouts, reset, and normal execution. External
interrupt entry and the public interface still use TypeScript. The
[model contract](../../../../docs/cpus/z80/model.md) describes those boundaries;
...
prefix fetching, reset, and retirement; the existing model contract still
describes external interrupt entry.
...
available to callbacks. The `interrupt external` declaration leaves NMI and
IM 0/1/2 entry in the adapter; that adapter uses the same decoder, refresh
action, and retirement boundary when it executes an IM 0 instruction.
...
All 698 documented forms now belong to this chapter, including their prefix
layouts, displacements, and complete instruction behavior. Reset and ordinary
execution also belong to the chapter, bringing model migration to five of six
milestones. External IRQ/NMI entry and the public interface remain in the
handwritten core.
