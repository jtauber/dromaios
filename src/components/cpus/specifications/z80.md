...
policy DECIMAL "replace DAA result flags" (original: 8, result: 8, carry: flag) {
  ...
  N = 0
  C = or(not(lessThan(original, u8($9A), unsigned)), carry)
}
...
family DAA "00 100 111" {
  ...
  carry = flag C
  subtract = flag N
  low = select(or(not(lessThan(and(original, u8($0F)), u8($0A), unsigned)), half), u8($06), u8(0))
  high = select(or(not(lessThan(original, u8($9A), unsigned)), carry), u8($60), u8(0))
