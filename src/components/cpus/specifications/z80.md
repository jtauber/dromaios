...
policy WORDADD "Z80 ADD word flags (H at bit 11)" (left: 16, right: 16, result: 16) {
  H = bit(xor(xor(left, right), result), 12)
  C = carry(left, right)
  N = 0
}
...
```cpu
policy DECIMAL "replace DAA result flags" (original: 8, result: 8, decimalCarry: flag) {
  S = negative(result)
  Z = zero(result)
  H = bit(xor(original, result), 4)
...
policy REPEATHALF "repeat half-carry correction" (repeatCounter: 8, subtract: flag) {
  H = equal(and(repeatCounter, u8(15)), select(subtract, u8(0), u8(15)))
}
