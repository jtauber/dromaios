...
export function chapterInterface(header: ChapterTokens, lines: readonly ChapterTokens[], state: StateFields,
  ...
  const types = new Set(["State", "StoredState", "Snapshot", "MemoryAccess", "Access", "Instruction", "StepRecord", "ResetRecord",
    "InterruptAccess", "InterruptInstruction", "InterruptRecord"]);
...
export function generateChapterInterface(module: string, state: StateFields, api: ChapterInterface): string {
...
export type ${name}Access = MemoryAccess | PortAccess;
export type ${name}Instruction = FetchedInstruction;
export type ${name}StepRecord = InstructionStep<${name}Snapshot, ${name}Access> | HaltedStep<${name}Snapshot, ${name}Access>;
...
export class ${name} {
  ...
  constructor(ram: Ram, initialState: ${name}State, ports?: BytePorts) {
    checkMemory(ram);
  ...
  interrupt(acknowledge: () => number): ${name}InterruptRecord { return this.#execution.interrupt(acknowledge); }
}
`;
}
