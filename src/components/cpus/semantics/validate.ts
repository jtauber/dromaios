import type { FlagExpression, FlagPolicy, InstructionDefinition, NumberExpression, Register, Statement, Width } from "./model.ts";
...
export function validateInstruction(definition: InstructionDefinition): void {
  ...
  function expression(expr: NumberExpression, scope: ReadonlyMap<string, Width>, where: string): Width {
    switch (expr.kind) {
      case "value": return scope.get(expr.name) ?? fail(where, `value ${expr.name} has not been captured in this scope`);
      case "literal": {
  ...
  function flagExpression(expr: FlagExpression, scope: ReadonlyMap<string, Width>, where: string): void {
    switch (expr.kind) {
  ...
  function policy(policy: FlagPolicy, args: Readonly<Record<string, NumberExpression>>, scope: ReadonlyMap<string, Width>, where: string): void {
    ...
    for (const { flag, value } of policy.updates) {
      if (flag.cpu !== cpu.name || flags?.kind !== "group" || flags.fields[flag.field]?.kind !== "flag") fail(where, `unknown flag ${flag.cpu}.${flag.field}`);
      if (assigned.has(flag.field)) fail(where, `duplicate flag update ${flag.field}`);
      assigned.add(flag.field);
      flagExpression(value, parameters, where);
    }
  }
  function steps(body: readonly Statement[], scope: Map<string, Width>, parent: string): void {
    body.forEach((step, index) => {
      ...
      switch (step.kind) {
        ...
        case "read-source": {
          const local = new Map<string, Width>();
