...
export function generateInstructions(cpu: "6502" | "8080" | "6809", definitions: Readonly<Record<string, InstructionDefinition>>, { bindOpcodes = false } = {}): string {
  ...
  const methods = Object.entries(definitions).map(([name, input]) => {
    ...
    const context = capabilities.size
      ...
      : "";
    const key = bindOpcodes ? `0x${Number(name).toString(16).padStart(2, "0")}` : JSON.stringify(name);
    return `  // ${JSON.stringify(`${cpu} ${definition.name}`)}\n`
      + `  ${key}(state: Cpu${cpu}State${context}): void {\n${lines.join("\n")}\n  },`;
  });
