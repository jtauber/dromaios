...
import { perform, readSource, resetDevices, writeLatch, addWrap, alignmentFault, select, subtract, bitAnd, bitOr, bitXor, capture, commitAddressUpdates, concat, cpuSymbols, extend, fetchWord, flagValue, literal, lowBit, readFlag, readMemory, readProgramMemory, readRegister,
  readNextAddress, selectTarget, not, reject, resolveAddress, shiftBits, signExtend, truncate, value, when, writeMemory, writeRegister, zero } from "../model.ts";
...
import { families, views, actions } from "../generated/68000.ts";
...
// Word and decimal families retain their distinct source-update and flag commit points.
const wordFamilies = Object.entries(families).filter(([name]) => name.startsWith("operandWord")).flatMap(([, entries]) => entries);
export const { definitions: wordArithmetic68000, opcodeAliases: wordArithmeticOpcodes68000 } = instructionAliases(wordFamilies);
const decimalFamilies = Object.entries(families).filter(([name]) => name.startsWith("operandDecimal")).flatMap(([, entries]) => entries);
export const { definitions: decimal68000, opcodeAliases: decimalOpcodes68000 } = instructionAliases(decimalFamilies);
