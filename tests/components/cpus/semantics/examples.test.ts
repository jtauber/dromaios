...
test("6800 CPX explains high-byte N/V, whole-word Z, and preserved carry in all four modes", () => {
  for (const mode of ["#word", "direct", "indexed", "extended"]) {
...
test("6800 unary explanations expose the three differences from the 6809", () => {
  const clear = description("6800", "CLR extended");
...
test("the review artifact is reproducible from the inert definitions and their accompanying prose", () => {
  ...
  assert.equal(instructionDefinitions.length, 10211);
});
