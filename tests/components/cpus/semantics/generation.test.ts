...
test("6800 comparison bodies read complete operands before registers, never write them, and preserve flags on read failure", () => {
  for (const [immediate, memory, register] of [[0x81, 0xb1, "a"], [0xc1, 0xf1, "b"], [0x8c, 0xbc, "x"]] as const) {
    for (const mode of ["Immediate", "Memory"] as const) for (const carry of [false, true]) {
      ...
      for (let failAt = -1; failAt < bytes.length; failAt++) {
        ...
        const execute = () => mode === "Immediate" ? motorola6800[immediate](observed, { fetchByte: read })
          : extended6800(motorola6800[memory])(observed, 0xffff, { readByte(address) { assert.equal(address, reads === 0 ? 0xffff : 0); return read(); } });
  ...
  motorola6800[0x11](new Proxy(state, { set() { assert.fail("CBA must not write registers"); } }));
