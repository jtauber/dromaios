...
test("Z80 shifts and rotates retain exactly the completed effects at every ordinary or IM 0 failure", () => {
  const forms = [
    ...([[0x07, "RLC"], [0x0f, "RRC"], [0x17, "RL"], [0x1f, "RR"]] as const).map(([opcode, name]) =>
      ({ bytes: [opcode], name, target: "a" as const, accumulator: true })),
    ...cbRows.filter(row => row.base < 0x40).flatMap(({ name, base }) => [
      ...transferColumns.map((target, code) => ({ bytes: [0xcb, base + code], name, target, accumulator: false })),
      ...indexes.map(({ prefix, index }) => ({ bytes: [prefix, 0xcb, 0xff, base + 6], name, target: index, accumulator: false })),
    ]),
  ];
  assert.equal(forms.length, 74);
  const failure = new Error("shift access failure");
  for (const { bytes, name, target, accumulator } of forms) for (const external of [false, true]) for (const bits of [0, 63]) {
    ...
    for (const address of memory ? [0xffff, 0, 1, 2, 0x4000] : [0xffff]) for (let failAt = -1; failAt < count; failAt++) {
      ...
      const result = expectedCb(name, 0, operand, state.flags);
      ...
      assert.deepEqual(cpu.snapshot(), snapshot({ ...state,
        ...
        ...(success || memory && failAt === count - 1 ? { flags } : {}),
