...
test("native CPU generation bootstraps without generated files and removes obsolete output", t => {
  ...
  const chapterNames = ["6502-load-store.ts", "68000-word-transfers.ts", "8008.ts", "8080.ts", "catalogue.ts", "interfaces.ts"];
  const chapterFiles = [...chapterNames, "state/8008.ts", "state/8080.ts"];
  ...
  const run = () => {
    ...
    assert.deepEqual(readdirSync(join(chapters, "state")).sort(), ["8008.ts", "8080.ts"]);
    ...
    assert.deepEqual(readdirSync(output).sort(), ["6502-interrupts.ts", "6502.ts", "6800.ts", "68000-arithmetic.ts", "68000-bits.ts", "68000-control.ts", "68000-decimal.ts", "68000-logic.ts", "68000-moves.ts", "68000-quick.ts", "68000-system.ts", "68000-transfers.ts", "68000-word-arithmetic.ts", "68000-word-moves.ts", "68000.ts", "6809.ts", "8008-cpu.ts", "8008-execution.ts", "8008-state.ts", "8008.ts", "8080-execution.ts", "8080-state.ts", "8080.ts", "8088-addressing.ts", "8088-alu.ts", "8088-arithmetic.ts", "8088-control.ts", "8088-stack.ts", "8088-strings.ts", "8088-transfers.ts", "8088-unary.ts", "8088.ts", "z80.ts"]);
    for (const cpu of ["6502-interrupts", "6502", "6800", "68000-arithmetic", "68000-bits", "68000-control", "68000-decimal", "68000-logic", "68000-moves", "68000-quick", "68000-system", "68000-transfers", "68000-word-arithmetic", "68000-word-moves", "68000", "6809", "8008-cpu", "8008-execution", "8008-state", "8008", "8080-execution", "8080-state", "8080", "8088-addressing", "8088-alu", "8088-arithmetic", "8088-control", "8088-stack", "8088-strings", "8088-transfers", "8088-unary", "8088", "z80"]) {
  ...
  const inspect = spawnSync(process.execPath, ["--input-type=module", "-e",
    `const { cpu8008StateDescription: state } = await import(${JSON.stringify(schema)});
     if (state.addressStack.length !== 8 || state.flags.fields.c.kind !== "flag") process.exit(1);`], { encoding: "utf8" });
