...
cpu 68000 {
  ...
  USP=34FFE000 SSP=56FFD000 PC=AB001000 IR=0000 faulted=false entry { kind=none vector=00 } interruptMask=2 halted=false tracePending=false
...
test("68000 parsing keeps full long registers and logical completion separate from physical memory bounds", () => {
  ...
  for (const source of [`${source68000} ${suffix}`, `${suffix} ${source68000}`]) {
    ...
    assert.deepEqual(machine.initialState, {
      ...
      pc: 0xab001000, ir: 0, faulted: false, entry: { kind: "none", vector: 0 }, halted: false, tracePending: false, interruptMask: 2, flags: { x: true, n: false, z: true, v: true, c: true, t: false, s: false },
    });
  }
...
test("8088 parsing requires and preserves pending traps and both interrupt delays", () => {
  for (const latch of ["waiting", "interruptDeferred", "recognitionDeferred", "trapPending"] as const) {
    ...
    for (const value of ["0", "1", "FALSE"]) assert.throws(() => parseMachine(set(source8088, latch, value)), /Expected true or false/);
    assert.throws(() => parseMachine(source8088.replace(`${latch}=false`, "")), /Missing fields/);
  }
});

test("68000 parsing retains the pending trace separately from T and STOP", () => {
  for (const value of ["true", "false"]) {
    const machine = parseMachine(set(source68000, "tracePending", value));
    assert.equal(machine.cpu, "68000");
    if (machine.cpu !== "68000") assert.fail();
    assert.equal(machine.initialState.tracePending, value === "true");
  }
  for (const value of ["0", "1", "yes"]) assert.throws(() => parseMachine(set(source68000, "tracePending", value)), SyntaxError);
  assert.throws(() => parseMachine(source68000.replace("tracePending=false", "")), /Missing fields in 68000: tracePending/);
});


test("68000 parsing requires IR and terminal-halt state for restorable address errors", () => {
  const machine = parseMachine(set(set(source68000, "IR", "FFFF"), "faulted", "true"));
  if (machine.cpu !== "68000") assert.fail();
  assert.equal(machine.initialState.ir, 0xffff);
  assert.equal(machine.initialState.faulted, true);
  assert.throws(() => parseMachine(set(source68000, "IR", "10000")), SyntaxError);
  for (const value of ["0", "1", "yes"]) assert.throws(() => parseMachine(set(source68000, "faulted", value)), SyntaxError);
  for (const field of ["IR=0000", "faulted=false"]) assert.throws(() => parseMachine(source68000.replace(field, "")), /Missing fields/);
});
