...
test("68000 mixed-size transfers and address arithmetic produce all 18 exact records and actual RAM accesses", t => {
  ...
  assert.deepEqual(reads.mock.calls.map(c => c.arguments), accesses.flatMap(a => a.kind === "read" ? [[a.address]] : []));
  assert.deepEqual(writes.mock.calls.map(c => c.arguments), accesses.flatMap(a => a.kind === "write" ? [[a.address, a.value]] : []));
  ...
  assert.deepEqual(continued.after, { ...records[17]!.after, ir: 0, pc: endAddress + 4, physicalPc: 0x203e });
});
