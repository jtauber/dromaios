...
function initialState(supervisor = false): Cpu68000Snapshot {
  return { d0: 0x11223344, d1: 0x55667788, d2: 0x99aabbcc, d3: 0xddeeff00,
    ...
    pc: 0xab002000, a7: supervisor ? 0x56009000 : 0x34008000, physicalPc: 0x2000, interruptMask: supervisor ? 7 : 2,
...
test("68000 frame example combines all seven new families in sixteen exact records in both modes", t => {
  for (const supervisor of [false, true]) {
    ...
    assert.deepEqual(reads.mock.calls.map(call => call.arguments), expected.filter(a => a.kind === "read").map(a => [a.address]));
    assert.deepEqual(writes.mock.calls.map(call => call.arguments), expected.filter(a => a.kind === "write").map(a => [a.address, a.value]));
