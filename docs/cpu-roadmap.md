...
8080 state, `MVI A,n`, `ADI n` and its flags, step records, reset, and lesson
setup are implemented, with TypeScript compiled to ES modules and tests run
by Node.js 24's built-in runner. The next small change adds `STA addr`. The
