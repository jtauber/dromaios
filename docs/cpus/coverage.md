...
| [Intel 8088](#8088) | 1979 | [29,000][intel-transistors] | [452](../../src/components/cpus/8088.ts) | [1,588](../../src/components/cpus/specifications/8088.md) | 1,163 | 124 / 291 | 42.6% | 2 / 6 |
...
model. The 8008, 8080, 6502, 6800, 6809, and Z80 also own all model milestones and generate their public
...
the six criteria below. The 8008, 8080, 6502, 6800, 6809, and Z80 are at **6 / 6**: instructions,
...
and its generated public interface with nested bank views.
...
The 8008, 8080, 6502, 6800, 6809, and Z80 have **zero handwritten implementation lines**, including their
...
The 8088 reaches **2 / 6**: complete stored state and derived register views/writes
are chapter-owned. Its partial instruction chapter also supplies register
selectors, arithmetic/status policies, segmented memory, and ModR/M addressing
to native bodies. Reset, prefix scanning, remaining group decoding, execution,
external entry, and the public class remain native. The 68000 remains
at **0 / 6** with a partial instruction chapter and TypeScript-owned state. Declarations checked against
an external schema do not earn stored-state credit.
...
**124 / 291 forms (42.6%)**: sixteen immediate MOV, eighteen accumulator ALU/TEST,
sixteen word INC/DEC, LAHF/SAHF, twelve ModR/M/absolute MOV/XCHG, and sixty
ModR/M or immediate ALU/TEST forms. Complete stored state, byte-register
views/writes, physical PC, and packed FLAGS earn **2 / 6 model milestones**.
Remaining native bodies consume its operand catalogues, arithmetic/status
policies, effective-address source, and memory read/write actions.
[Chapter tests](../../tests/components/cpus/semantics/literate-8088.test.ts) verify exact
ownership and reserved-bit behavior; [integration tests](../../tests/scripts/8088-chapter-integration.test.ts)
prove edits reach public state, snapshots, machine parsing, migrated instructions,
and native consumers. Reset, prefix scanning, remaining families, execution,
and the public interface remain.
...
- [8088 definitions](../../src/components/cpus/semantics/definitions/8088.ts), now
  partly authored in the [8088 chapter](../../src/components/cpus/specifications/8088.md):
...
| Scope | Lines |
| --- | ---: |
| Two remaining handwritten CPU core files | 1,056 |
| CPU-specific instruction definition files | 1,169 |
| Other authored CPU source: shared helpers, state schemas, semantic model, builders, validation, generator, reporter, and literate front end | 5,091 |
| **All authored TypeScript under `src/components/cpus`, excluding both generated directories** | **7,316** |
| Authored CPU chapters (Markdown, including prose and formal blocks) | 13,756 |
| CPU generation scripts (`generate-cpu-semantics.ts` and `generate-cpu-chapters.ts`) | 130 |
| Generated executable CPU output, counted separately | 288,618 |
| Generated chapter data, catalogues, and entry-point metadata, counted separately | 1,322,721 |
...
is now **121 lines**; the literate front end is **1,658 lines**.
...
The 8088 segmented-operand slice reaches **124 / 291 chapter forms (42.6%)**.
Its **72 additional forms** cover twelve MOV/XCHG transfers and sixty ModR/M
or immediate ALU/TEST forms. The chapter owns effective-address capture,
register/memory selection, and low-first byte accesses with logical offset wrap.
It replaces **1,937 specialized transfer/ALU bodies** with reusable sources and
actions. Native consumers share its address and memory helpers. Prefix scanning,
the remaining families, and execution boundaries still need migration; model
milestones remain **2 / 6**.

The core falls from **524 to 452 lines** and the native definition file from
**609 to 531**. Numeric source/family inputs, memory address projections, and
composed byte matches add general language support. Including that support,
authored CPU TypeScript falls from **7,410 to 7,316 lines** (**94 fewer**).
The 8088 chapter grows from **664 to 1,588 lines**, with **1,163 formal
lines**, bringing total maintained CPU source from **20,372 to 21,202 lines**.
This migration reduces handwritten TypeScript and generated executable code;
maintained source including the fuller executable explanation still grows.

Independent instruction and failure-order tests remain in place. Chapter-edit
checks change segment selection and word-read offsets through public execution,
including remaining native consumers. Before/after public execution matches
**16,384 first-byte cases**, **23,040 ModR/M cases**, **1,536 reset/interrupt cases**,
and **13,795 injected failures**, including snapshots inside callbacks. Coverage
measures literate ownership; all documented opcode forms remain implemented.
...
reader is also now generated only by its chapter test. Twenty-three production readers provide the 8008's PC/HL, 8080's BC/DE/HL/NEXT, 6502's STATUS/NEXT,
6800's CC/NEXT, 6809's D/CC/STACK/NEXT, and Z80's six pair views, F, AF, and NEXT. One generated indexed decoder is shared by
6809 base-page and prefixed bodies. Readers earn no separate opcode coverage credit.
...
[Chapter](../../src/components/cpus/specifications/8088.md) ·
[Remaining core](../../src/components/cpus/8088.ts) ·
...
```sh
# Fully generated CPUs (8008, 8080, 6502, 6800, 6809, Z80) have no handwritten core file.
wc -l src/components/cpus/{8088,68000}.ts
```
