...
| [Intel 8080](#8080) | 1974 | [6,000][intel-transistors] | [0](../../src/components/cpus/specifications/8080.md) | [1,601](../../src/components/cpus/specifications/8080.md) | 511 | 244 / 244 | 100% | 6 / 6 |
...
| [MOS 6502](#6502) | 1975 | [3,510][6502-transistors] | [0](../../src/components/cpus/specifications/6502.md) | [1,759](../../src/components/cpus/specifications/6502.md) | 601 | 151 / 151 | 100% | 6 / 6 |
...
| [Motorola 68000](#68000) | 1979 | [68,000][68000-transistors] | [0](../../src/components/cpus/specifications/68000.md) | [8,313](../../src/components/cpus/specifications/68000.md) | 4,836 | 36,029 / 36,029 | 100% | 6 / 6 |
...
The 6502 adds chapter-owned reset memory accesses and named IRQ/NMI entry; its
public interface is generated too. The 6800 reuses vector execution with declared
WAI waiting and wake-up rules. Its chapter owns reset, normal execution, IRQ/NMI
recognition, frame reuse, and its generated public interface. The 6809 extends
that shared boundary with named wait modes, NMI arming, masked SYNC release,
CWAI frame reuse, and FIRQ short frames. Its chapter now also owns all six areas
and generates its public interface. The Z80 also owns all six areas: its chapter
selects full-encoding validation, refresh/retirement, NMI and IRQ mode delivery,
and its generated public interface with nested bank views. The 8088 adds segmented
execution, TEST/ESC connections, ordered vector offers, and physical-PC metadata.
The 68000 adds word execution, logical-to-physical bus mapping, exception frames,
and recovery from transfer failures, with a generated public interface.
...
The Z80 is at **6 / 6**. Its chapter owns recognition gates, acceptance effects,
and NMI/IRQ mode-specific entry as well as instructions and normal execution.
Its public class, bank types, nested snapshots, and machine integration metadata
are generated. No handwritten Z80 implementation or separate model document remains.
The 8088 reaches **6 / 6**. Instructions, stored state, register views/writes,
...
remains. The 68000 reaches **6 / 6**: all instruction definitions and encodings,
stored state, derived views/writes, reset, normal execution, and external events
are chapter-owned. This includes register
...
after modeled faults. Word execution declares fetch commits, encoded fields,
stopping, retirement, and trace scheduling. Its event contract owns exception
frames, interrupt recognition and acknowledgement, initial-fetch recovery, and
terminal halt after a second fault. All eight models now reach **6 / 6**. The
68000 public interface, connection wrapper, state types, and catalogue now
generate too. All eight chapters are their processors’ sole authored implementation
and model contract; shared TypeScript runtime services remain.
...
**151 documented instruction forms**, the complete stored-state schema, and
packed status. Decimal correction, invalid BCD inputs, two-write memory updates,
branch displacement signs, JSR operand/stack overlap, and RTI restoration order
are explicit. External entry consumes its status view and I-update policy; that
shared use earns no extra opcode credit. Reset, execution, IRQ/NMI recognition
and entry, public types, and snapshots also come from the chapter; shared runtime
services supply recording and guards. [Chapter tests](../../tests/components/cpus/semantics/literate.test.ts)
check the exact inventory; [arithmetic tests](../../tests/components/cpus/semantics/literate-6502-arithmetic.test.ts)
and [control/stack tests](../../tests/components/cpus/semantics/literate-6502-control.test.ts)
check formal edits, diagnostics, and effect ordering. [Integration tests](../../tests/scripts/chapter-model-integration.test.ts)
verify that state edits reach construction, snapshots, and machine parsing, and
that status-policy edits reach both instruction and external interrupt entry.
[Lifecycle chapter tests](../../tests/components/cpus/semantics/literate-6502-lifecycle.test.ts)
change reset addresses/effects, entry vectors, masks, source names, and public
names, retaining access-failure behavior and Markdown diagnostics. Model and API
contracts are consolidated into the chapter; no separate 6502 model document remains.
...
generate PC/HL readers, selected-PC writes, and reset effects. Its `interface`
declaration generates the public class, readonly types, and snapshot assembly.
Its execution declaration
binds the counter, stopped latch, reset/acceptance actions, and fetch/retirement
...
**all 244 documented forms**, including word arithmetic and transfers, stacks,
control flow, decimal adjustment, packed status, I/O, and interrupt controls.
It also owns the complete stored schema, BC/DE/HL views and PC writes, reset,
normal execution, external interrupt acceptance, and generated public interface.
Its model and API contracts now live beside the executable definitions. No
handwritten core, state adapter, instruction adapter, or 8080-specific branch
in the remaining status/decimal builders remains. The same byte runtime serves
8008 unconditional acceptance and 8080 enable/deferral recognition, with chapter
policies preserving each CPU's callback-validation and retirement rules.

The [6800 chapter](../../src/components/cpus/specifications/6800.md) owns **all 197
forms**, including arithmetic, decimal adjustment, unary operations, flag controls,
branches, calls, stacks, WAI/SWI/RTI, and their complete addressing and encodings.
CPX retains the original chip's separate high-byte N/V calculation; CLR avoids
reading its destination, TST clears C, and right shifts replace V. Reusable
chapter actions express byte pushes, word pushes, calls, and interrupt frames.
WAI, SWI, and external entry share the same frame rules; external recognition
comes from the execution declaration, with masks and vectors explicitly selected.
[Chapter tests](../../tests/components/cpus/semantics/literate-6800.test.ts) verify
the exact inventory and formal edits; [action tests](../../tests/components/cpus/semantics/literate-actions.test.ts)
check argument capture, isolated scopes, nested effects, and diagnostics.
[Integration tests](../../tests/scripts/chapter-model-integration.test.ts) show
schema changes reaching construction/snapshots/machine parsing and status
changes reaching instruction and interrupt frames. [Lifecycle tests](../../tests/components/cpus/semantics/literate-6800-lifecycle.test.ts)
edit reset effects, waiting/wake policies, vectors, masks, and public/source names,
then run the generated public CPU. All six model areas and the interface come
...
The [6809 chapter](../../src/components/cpus/specifications/6809.md) owns **all 268
forms**, including addressing, prefix and transfer-postbyte decoding, register
masks, waiting, software interrupts, and returns. Its complete stored schema and
D/CC views and writes are joined by reset, execution, IRQ/FIRQ/NMI recognition,
and external frame selection, reaching **6 / 6 model milestones**. The public
class, records, catalogue, and machine metadata are generated; the handwritten
core and state/definition adapters are removed. Its model contract and hardware
background now live in the chapter, with no separate model document.

Named opcode pages dispatch base and prefixed forms; a shared indexed source
owns base selection, signed offsets, auto-updates, S arming, PC-relative fetching,
indirect reads, and unsupported postbytes. EXG/TFR use effect matches and writable
views for D, CC, and S. Reserved selectors are explicit; invalid or mixed-width
pairs stop before register reads. Stack actions retain capture order, live
pointers, complete-field commits, and distinct NMI-arming schedules. Named choice
reads and writes distinguish SYNC and CWAI, including saved-frame reuse.

[Chapter tests](../../tests/components/cpus/semantics/literate-6809.test.ts) check
the exact inventory, arithmetic/addressing behavior, and formal edits to state,
views, wait modes, and software vectors. [Transfer probes](../../tests/components/cpus/semantics/6809-registers.test.ts)
check every postbyte and register/flag access order; [mask probes](../../tests/components/cpus/semantics/masked-stacks.test.ts)
check every mask and failed access. [Indexed tests](../../tests/components/cpus/semantics/literate-6809-indexed.test.ts)
retain postbyte and failure expectations. [Language effect tests](../../tests/components/cpus/semantics/literate-effects.test.ts)
exercise writable views, choices, unsupported selectors, and effect matches on
a different schema. [Page tests](../../tests/components/cpus/semantics/literate-pages.test.ts)
check isolated opcode spaces and exact fetching. [Integration tests](../../tests/scripts/chapter-model-integration.test.ts)
show chapter edits reaching the public core, machine parser, indexed behavior,
and prefix dispatch, including SWI2/SWI3. [Lifecycle tests](../../tests/components/cpus/semantics/literate-6809-lifecycle.test.ts)
change reset vectors and effects, frame shapes, masks, arming, wait and resume
choices, and derived snapshot fields through the public CPU. A renamed model
...
**all 291 documented forms (100%)** and **6 / 6 model milestones**. Its
[execution contract](../../src/components/cpus/specifications/8088.md#reset-and-execution-lifecycle)
declares segmented fetching, prefixes, WAIT continuation, trap/fault delivery,
retirement, and ordered INTR/NMI offers. The public class, connections, records,
snapshots, and integration metadata are generated from this complete chapter.
[Chapter tests](../../tests/components/cpus/semantics/literate-8088.test.ts) verify
instruction ownership; [lifecycle tests](../../tests/components/cpus/semantics/literate-segmented-execution.test.ts)
check policy edits, live callbacks, retained failures, and rejected contracts.
[Offer tests](../../tests/components/cpus/semantics/literate-vector-offers.test.ts)
change gates, vectors, acceptance effects, names, and views, including a renamed
model. [Public integration tests](../../tests/scripts/8088-chapter-integration.test.ts)
verify chapter edits through generation, construction, machine parsing, memory
bounds, and execution.

The [68000 chapter](../../src/components/cpus/specifications/68000.md) owns
**all 36,029 documented forms (100%)**. The final 1,867 comprise 1,285 control
forms, 396 MOVEP/MOVEM forms, and 186 status/system forms. The complete chapter
covers **54,008 operation words**, including the software emulator lines.
...
Complete instructions, stored state, derived views/writes, reset, normal execution,
and external events earn **6 / 6 model milestones**.
...
word execution contract now owns fetching, decoded inputs, retirement, and trace
scheduling. The event contract owns frame layouts, acknowledgement, and fault
recovery. The native core retains public records, snapshots, and connection
wrappers; its definition adapter groups chapter families for generated calls.

[Register tests](../../tests/components/cpus/semantics/68000-registers.test.ts)
retain independent width, bank, alias, live-read, and failed-effect expectations.
[Chapter tests](../../tests/components/cpus/semantics/literate-68000.test.ts)
check production ownership, all status values, memory faults, and formal edits.
...
contains **10,111 generated bodies** for instructions and reusable actions,
...
- [6502 chapter](../../src/components/cpus/specifications/6502.md):
  ...
  result writeback while N/Z follow it. The chapter also owns the eight
  conditional branches and absolute/indirect JMP. Branches fetch before testing flags and read/write
  ...
  PHP/PLP, the seven flag changes, and NOP add ten chapter bodies.
  Status packing uses the chapter-owned view;
  ...
  follows BRK's frame convention using the same status view and I-update policy,
  with the saved B marker clear. The accumulator
...
- [8080 definitions](../../src/components/cpus/specifications/8080.md):
  ...
  chapter bodies with the same access order as their Z80 counterparts. Word operands
  ...
  using chapter pair operands and native bit patterns.
  ...
  add six chapter-owned accumulator-transfer bodies. They capture the complete
  address before reading A or memory;
  stores never read the destination and loads write A only after a successful
  read. Flags are never accessed. XTHL/XCHG add two chapter-owned exchange bodies.
  XTHL captures HL before SP, reads low/high, writes high/low, then
  ...
  jumps, JMP, and PCHL add ten chapter bodies with native condition selectors.
  Immediate targets are fetched completely before flags
  ...
  explicit descending-stack effects: predecrement before writes, increment
  ...
  add eight bodies. PSW packing and restoration use the chapter's explicit
  reserved-bit layout;
  ...
  DI/EI complete all 244 bodies and forms. EI sets the enable latch and requests IRQ deferral; the
  CPU commits that request only at successful retirement.
- [6800 chapter](../../src/components/cpus/specifications/6800.md):
  All 197 documented forms have complete bodies, including addressing. These
  comprise 44 unary forms, 13 comparisons, 32 logical forms, 30 transfers,
  34 binary arithmetic forms, 17 branches/jumps, eight stack/call/return forms,
  sixteen decimal/status/flag/pointer/NOP forms, and WAI/SWI/RTI.
  The original mnemonics ORAA/ORAB and LDAA/LDAB are retained. Arithmetic sets
  flags before writeback; addition replaces H and subtraction preserves it.
  Transfers set N/Z/V after successful writes, including both bytes of a word.
  SP names the next free byte: push writes before decrementing; pull increments
  before reading. Word pushes send low/high and pulls read high/low. Calls
  resolve their targets before stacking; completed effects remain on failure.
  Chapter actions share the byte-stack rule across calls and frames. Frames
  capture each field at its turn, save the old I, and restore CC before registers.
  The external-entry helper uses the same action as SWI, reusing an existing WAI
  frame before masking IRQ and reading its vector; it adds no opcode credit.
- [6809 chapter](../../src/components/cpus/specifications/6809.md):
  CMPA/B/D/X/Y/U/S across immediate/direct/indexed/extended addressing cover
  28 complete chapter forms across base and prefixed pages.
  ...
  LDD/LDX/LDY/LDU/LDS and their stores cover 35 complete chapter forms,
  including addressing and both base and prefixed encodings.
  ...
  their complete displacement. The chapter owns all branches, sharing conditions
  and writeback across short and long encodings while retaining standalone LBRA. JMP covers three complete chapter forms.
  ...
  successful fetch permits complete flag replacement.
  TFR/EXG are two complete chapter bodies, each validating and dispatching its
  52 legal same-width postbytes. They capture both registers before
  ...
  A chapter-owned system-stack action reuses that construction for external entry
  ...
  complete all 268 forms. Chapter-authored wait-mode choices retain CWAI frame reuse;
  restored E selects RTI's full or short frame before final NMI arming.
- [Z80 chapter](../../src/components/cpus/specifications/z80.md):
  The chapter owns all 698 forms: 252 unprefixed, 248 ordinary CB, 58 ED,
  78 DD/FD, and 62 indexed CB.
  Unprefixed families include byte and word transfers,
  arithmetic and adjustments, exchanges, stack/control flow, accumulator rotates,
  decimal and carry controls, ports, DI/EI, NOP, and HALT. Indexed families
  reuse its pair reads/writes, byte arithmetic and CB memory actions, bit masks,
  and word-addition policy.
  The inherited base class, native base encodings, and redundant builders are
  removed. Prefix layouts and displacement fetching are chapter-owned;
  reset, refresh commitment, and retirement are now chapter-bound. RLCA/RRCA/RLA/RRA write A and C before clearing N/H;
  S/Z/PV are preserved. All seven documented
  CB shift/rotate families cover 70 register, (HL), (IX+d), and (IY+d) forms.
  Each chapter memory action serves all three memory operands.
  ...
  flags. BIT/RES/SET cover 240 forms, completing the documented
  ...
  on a failed write. Ordinary and indexed CB encodings, masks, and effects are chapter-owned.
  Nested pages capture displacement before the final opcode, with both marked
  as ordinary reads. The old builders and indexed dispatch tables are removed.
  INC/DEC cover 20 chapter-owned byte forms; two memory actions serve both
  HL and indexed operands. They preserve C, set H
  from carry/borrow, derive P/V from signed overflow, and apply flags before
  writeback. The chapter also owns all 63 byte-transfer matrix slots and eight
  immediate loads, retaining source-before-destination access. Three shared
  indexed transfer families cover 30 IX/IY forms, retaining real
  ...
  from 23 opcode bodies: seven base forms, eight ED absolute pair transfers,
  and eight IX/IY immediate, absolute, and SP forms. ED's HL transfers retain
  the unprefixed behavior. Address fetching, complete source capture, low-first
  ...
  bodies and forms. Indexed arithmetic reuses chapter pair descriptions
  and its ADD policy; IX/IY use stored words. Chapter ADC/SBC read C after both operands.
  ...
  addresses have chapter-owned patterns and bodies. Pair addresses
  ...
  EX DE,HL and EX (SP),HL/IX/IY are chapter-owned; IX/IY forms share a
  body through writable register bindings. Stack exchanges capture the register before SP, read low/high,
  ...
  forms. Chapter stack actions serve ordinary pushes/pops and calls/returns,
  retaining complete source capture, delayed pop writeback, and byte-level failures.
  DAA, CPL/SCF/CCF, NOP/HALT, and PUSH/POP AF add eight more bodies. AF uses
  the chapter's F view and complete flag-replacement action; DAA retains incoming N
  after result writeback. Prefix decoding, supplied instructions, refresh, and
  retirement retain their existing contracts. Chapter call/push actions also
  serve external interrupt entry; the unused runtime stack helper is removed.
  ...
  and all 24 I/O forms are chapter-owned: immediate ports capture old A before fetching, register ports use BC,
...
- [8008 definitions](../../src/components/cpus/specifications/8008.md):
  All 72 byte ALU forms are integrated: AD/AC/SU/SB/ND/XR/OR/CP with each
  register, memory, and immediate source. Literate families share one body per
  operation, with typed S/Z/P/C policies and explicit C-before-A reads for AC/SB.
  Native sources follow A/B/C/D/E/H/L/M; M masks H:L to `3FFF` without
  changing H or L. Comparison never writes A. The chapter's execution contract
  preserves the selected address slot and supplied-byte fetches that leave
  every address register unchanged.
  ...
  literate forms: capture A, read incoming C only for RAL/RAR, then write A
  before C. S/Z/P are preserved.
  All 63 register/memory transfers and eight immediate loads add 71 bodies using
  the literate chapter, with native A/B/C/D/E/H/L/M selectors,
  ...
  and supplied-byte behavior with the chapter-bound runtime. INP/OUT add all 32 port forms,
  ...
  PC/HL views, selected-PC writes, reset effects, and interrupt acceptance.
  Its execution declaration selects fetching, stopping, rejection, and retirement
  policies; shared runtime services enforce the contract and assemble records.

- [8088 chapter](../../src/components/cpus/specifications/8088.md):
  ...
  Chapter byte views read AL/AH and their siblings from stored words;
  write actions reread the word at writeback and retain the current other half.
...
- [68000 definitions](../../src/components/cpus/specifications/68000.md):
...
**439-line** document, which was outside this count. That consolidation changes
documentation ownership and counting, without changing generated CPU code.
The historical background, architecture tour, physical-operation guide, and
worked explanation add another **317 chapter lines**, also without changing
generated behavior. This measure includes the literature, not just executable
definitions.

The shared model catalogue accounts for **32 lines** in the CPU total; the
machine parser, outside this count, shrinks from **225 to 191 lines**. It no
longer repeats per-model schema selection, RAM sizes, or completion bounds.
The new language and generation support is shared; moving further CPUs will
test its reuse. Zero handwritten 8008 lines does not mean zero generated code
or zero shared runtime machinery.

The final 8080 migration removes its **75-line public core**, **9-line state
adapter**, and **31-line instruction adapter**. Pair operands, numeric selection,
Boolean predicates, and flag replacement reuse the existing representation.
The declared retirement destination grants IRQ deferral without a CPU-name check.
Removing unused 8080 status/decimal branches offsets most language support;
all authored CPU TypeScript falls from **9,506 to 9,401 lines**, a net reduction
...
behavior. At that point, maintained CPU source (TypeScript, chapters, and the
two generation scripts) reached **12,881 lines**; prose counts toward that total.
Both the 8008 and 8080 now have a single authored implementation source.

The 6502 state and 77-form migration reduces its instruction-definition file
from **241 to 148 lines** and its state adapter from **15 to 9 lines**. No new
...
maintained CPU source reached **13,458 lines**, including prose. The generated
executable output grows by 64 lines because decimal intermediates are named
explicitly. The core and model-milestone count stayed unchanged in that slice.

The final **26 stack/control/status forms** bring the 6502 chapter to **151 / 151
(100%)** and **3 / 6 model milestones**. Its definition adapter shrinks from **84
to 31 lines**, its state adapter from **9 to 5 lines**, and one net parser line
exposes the existing signed-widening operation. Authored CPU TypeScript falls
by **56 lines**, from **9,195 to 9,139**, without new runtime code. The chapter
reaches **1,380 lines**, including **548 formal lines**; total maintained CPU
source reached **13,813 lines**, including prose. Generated executable output grows
by 18 lines from explicit status intermediates and named JMP sources. The
101-line core remained for the lifecycle migration.

That lifecycle migration removes the **101-line core**, **5-line state adapter**,
and **31-line definition adapter**, reaching **zero handwritten 6502 implementation**
and **6 / 6 model milestones**. Shared memory-action, named-entry, and interface
support offsets those removals: authored CPU TypeScript grows by **39 lines**,
from **9,139 to 9,178**. This is the first chapter with reset bus reads and
vector entry; its processor rules now come entirely from the chapter. The
chapter reaches **1,747 lines**, including **601 formal lines**, and replaces
the separate **536-line model document**, which was outside this source count.
Total maintained CPU source reaches **14,219 lines**, including literature.
Its generated public class has **48 lines**, execution bindings **27**, and state
module **18**. The old standalone addressing readers move to test-only probes,
reducing generated production output even as the public adapter becomes generated.

The first 6800 chapter removes **52 authored TypeScript lines**, reducing the
CPU core from **213 to 203**, its definition file from **107 to 76**, and its
state module from **16 to 5**; shared-source changes have no net line cost.
The chapter contributes **432 lines**, including **203 formal lines**, without
new language or runtime machinery. Authored CPU TypeScript falls from **9,178
to 9,126**; total maintained CPU source becomes **14,599 lines**, including prose.
Generated bodies now include addressing separately for each migrated opcode,
while the remaining TypeScript definitions still share resolved-memory bodies.

Completing the 6800 instruction chapter removes another **123 authored TypeScript
lines**: the core falls from **203 to 107**, and its definition adapter from
**76 to 5**. Shared action composition adds **44 lines** across the representation,
validator, generator, reporter, and front end. Authored CPU TypeScript then
reached **9,003 lines**. The chapter reaches **1,457 lines**, including **722 formal lines**;
total maintained CPU source reached **15,501 lines**, including literature and generation
scripts. Complete generated bodies owned every opcode's addressing; the remaining
adapter collected chapter families and exposed the external-entry action.

Completing the 6800 model removes its **107-line core**, **5-line state adapter**,
and **5-line definition adapter**, reaching **zero handwritten CPU-specific
TypeScript** and **6 / 6 model milestones**. Shared waiting support and catalogue
cleanup give a net **98-line reduction**, from **9,003 to 8,905 authored TypeScript
lines**. Reset and IRQ/NMI reuse the vector runtime, with WAI suspension selected
by the execution declaration and wake-up effects defined by the entry action.
The chapter reaches **1,789 lines**, including **753 formal lines**, and absorbs
the separate **477-line model document** along with historical and hardware
background. Total maintained CPU source reached **15,735 lines**, including literature
and generation scripts. Shared runtime code still supplies fetching, recording,
and guards; all 6800 policy is authored in the chapter.

The first 6809 chapter slice removes its handwritten stored schema, D/CC rules,
and 26 immediate bodies. Its indexed and prefixed bodies remain; combining their
bindings with the chapter adds some adapter code. Named-choice declarations add
shared storage syntax. The net change is **11 more authored TypeScript lines**,
from **8,905 to 8,916**. The chapter adds **584 lines**, including **301 formal
lines**. Total maintained CPU source reached **16,330 lines**, including chapters and
generation scripts. Further migration can remove the remaining operand wrappers
and CPU-specific builders; that first slice did not reduce the overall footprint.

The next 6809 slice moves 75 more forms into the chapter, reaching **163 / 268**.
It removes native unary register bodies, base-page branch and call builders,
register/flag bodies, and the Motorola decimal specialization. Indexed JSR/JMP
...
maintained CPU source reached **17,196 lines**, including prose and generation scripts.
...
The opcode-page slice adds all 45 ordinary prefixed 6809 forms, reaching
**256 / 268 (95.5%)**. Named page declarations replace handwritten prefix tables;
short and long branches share their chapter bodies. The remaining Motorola
comparison, transfer, and long-branch builders and operand wrappers are removed.
The core falls from **244 to 195 lines**, and authored CPU TypeScript falls
by **132 lines**, from **8,695 to 8,563**, including generic page support.
The chapter reaches **1,845 lines**, including **982 formal lines**, and
CPU generation scripts total **129 lines**. Total maintained CPU source is
**17,239 lines**, including literature and generation scripts. The remaining
twelve instruction forms and the reset/execution/event contracts still need
chapter ownership; the wider model remains at **2 / 6 milestones**.

The final instruction slice moves the remaining twelve 6809 forms into the
chapter, reaching **268 / 268 (100%)** and **3 / 6 model milestones**. Effect
matches replace the transfer-postbyte table and 104 specialized bodies; writable
views keep D/CC/S write rules explicit. Chapter actions share masked-stack and
interrupt-frame effects, allowing removal of the unused frame/mask builders.
The core falls from **195 to 149 lines**, and authored CPU TypeScript falls
by **140 lines**, from **8,563 to 8,423**, including the language additions.
The chapter reaches **2,205 lines**, including **1,265 formal lines**.
Total maintained CPU source is **17,459 lines**, including literature and
generation scripts. At that stage reset, execution, external recognition, and
the public interface remained for the final model migration.

The completed 6809 model removes its **149-line core**, **5-line state adapter**,
and **5-line definition adapter**, reaching **6 / 6 milestones** and zero
handwritten processor-specific implementation. Reset and external entry actions,
named-choice waiting, arming gates, and masked resume clauses bind to the shared
vector runtime. The same runtime still serves the 6502 and 6800. Authored CPU
TypeScript falls from **8,423 to 8,314 lines**, a net reduction of **109**
including the language and public-interface changes. The chapter is now
**2,566 lines**, including **1,322 formal lines**; the separate **549-line**
model document is removed. Total maintained CPU source is **17,711 lines**,
including chapters and generation scripts. The increased literature count
includes the consolidated public contracts and hardware background.

The first Z80 chapter owns both stored banks and **159 / 698 forms (22.8%)**.
Its **563 literature lines**, including **269 formal lines**, replace
ordinary byte definitions and dispatch hooks. Native indexed arithmetic reuses
chapter actions; snapshots use its pair views and PUSH/POP AF use its status
rules. Obsolete byte builders, encodings, and runtime pair readers are removed.
Authored CPU TypeScript falls from **8,314 to 8,176 lines**, a net reduction
of **138** including bank/choice language support. Total maintained CPU source
is **18,136 lines**, including chapters and generation scripts.
[Chapter tests](../../tests/components/cpus/semantics/literate-z80.test.ts) check the
independent inventory, both banks, all pair values, and every packed F byte;
[integration edits](../../tests/scripts/z80-chapter-integration.test.ts) reach
construction, machine parsing, snapshots, ordinary/indexed arithmetic, and stacks.

The second Z80 slice moves all remaining unprefixed instructions into the
chapter, reaching **252 / 698 forms (36.1%)** and **2 / 6 model milestones**.
Native prefixed forms reuse chapter pair reads/writes and word-addition flags.
The obsolete inherited base, encoding inventory, pair mapping, decimal builder,
and unused Intel builders are removed. Authored CPU TypeScript falls from
**8,176 to 7,783 lines**, a net reduction of **393** including flag-group
exchange support. The chapter grows to **1,147 lines**, including **647 formal
lines**; total maintained CPU source becomes **18,327 lines**. This broader
count includes the explanatory literature. Integration edits also reach native
prefixed pair writes, ordinary words, relative branches, stack actions, and DAA.
Independent effect-order probes and old/new comparisons retain full records,
state, bus events, callback snapshots, and partial-failure behavior.

The third Z80 slice moves the entire ordinary CB page into the chapter,
reaching **500 / 698 forms (71.6%)**. Its `xx yyy rrr` encodings, shift/test
policies, and bit-mask catalogue replace native builders and ordinary CB
bindings. Indexed CB callers reuse the chapter's memory actions and masks;
those 62 indexed forms still await complete chapter-owned decoding. Separate
generated page tables preserve the core's pre-execution validation, PC/refresh
rules, and supplied-interrupt fetching. Model milestones remain **2 / 6**.
Authored CPU TypeScript falls from **7,783 to 7,745 lines**, a net reduction
of **38** including reusable page bindings. The chapter has **1,455 lines**,
including **837 formal lines**; total maintained CPU source is **18,597 lines**.
Integration edits prove that changing the CB prefix, shift flags, or bit masks
changes public execution, including indexed and interrupt-supplied forms.

The fourth Z80 slice moves all 58 ED forms into the chapter, reaching
**558 / 698 forms (79.9%)**. Word arithmetic/transfers, special registers,
nibble rotations, block copies/searches, all port forms, interrupt modes, and
RETN/RETI now have chapter-owned encodings and ordered effects. Block directions
and repeat bindings share bodies without hiding the per-iteration boundary.
The chapter exposes fixed-count logical shifts and RETI notification requests;
the core retains notification delivery after retirement. Native ED dispatch
and builders are removed. Model milestones remain **2 / 6**; all 140 remaining
forms are indexed instructions.
Authored CPU TypeScript falls from **7,745 to 7,541 lines**, a net reduction
of **204** including language support and numeric-capture generation fixes.
The core falls from **355 to 313 lines**, and the chapter reaches
**1,923 lines**, including **1,143 formal lines**. Total maintained CPU source
is **18,861 lines**. Independent access-order probes retain live rereads,
flag-object replacement, partial failures, and retirement notification. Chapter
mutation tests change ED prefix selection, word flags, nibble shifts, block
steps/repetition, port flags, modes, and RETI notification through public
ordinary and interrupt-supplied execution.

The fifth Z80 slice moves all **140 indexed forms** into the chapter, reaching
**698 / 698 forms (100%)** and **3 / 6 model milestones**. DD/FD word and byte
families share writable index-register bindings and displacement sources.
DD/FD CB pages declare displacement-before-opcode reads and their fetch kinds;
the generated decoder captures bytes without state effects and binds the body
for execution after the core's boundary commitment. Native indexed dispatch,
all 84 remaining native bodies, and the obsolete Intel builders are removed.
Authored CPU TypeScript falls from **7,541 to 7,369 lines**, a net reduction
of **172**, including the shared page-layout validation and generation.
The core falls from **313 to 224 lines**. The chapter reaches **2,286 lines**,
including **1,415 formal lines**; total maintained CPU source becomes
**19,053 lines**. That broader count includes the explanatory literature.
Independent inventory and access-order probes cover all forms. Chapter mutation
tests change prefix paths, displacement arithmetic, and opcode-fetch classification
through ordinary and interrupt-supplied public execution. Before/after comparisons
match 43,008 ordinary/prefixed instructions, 10,240 supplied instructions, 192
reset/IRQ/NMI cases, and 39,942 injected access failures, including callback snapshots.
At that point, reset, execution boundaries, external events, and public-interface
generation remained to migrate.

The sixth Z80 slice moves reset and normal execution into the chapter, reaching
**5 / 6 model milestones**. Reset preservation, complete-encoding validation,
PC commitment, refresh increments, HALT, deferral, and RETI notification are
bound to the shared decoded runtime. External IRQ/NMI entry uses the same guard,
decoder, fetch action, and retirement while its acceptance policy remains native.
The core falls from **224 to 143 lines**, and the **5-line definition adapter**
is removed. Adding the runtime and checked language contract brings authored
CPU TypeScript from **7,369 to 7,445 lines**. The chapter reaches **2,366 lines**,
including **1,453 formal lines**; total maintained CPU source is **19,209 lines**.

[Execution-language tests](../../tests/components/cpus/semantics/literate-decoded-execution.test.ts)
exercise a differently named CPU with renamed fields, an eight-bit bus, both
flat and prefixed decoding, retirement, notification failures, and rejected
policy combinations. [Runtime tests](../../tests/components/cpus/decoded-execution.test.ts)
check word ordering and live-counter reads. Public chapter edits change reset,
refresh, and retirement destinations through ordinary and supplied execution.
Before/after comparisons retain the same 43,008 ordinary/prefixed cases,
10,240 supplied instructions, 192 reset/IRQ/NMI cases, and 39,942 failures,
including snapshots inside callbacks. External entry and a generated public
interface remained at that stage.

The final Z80 slice completes **6 / 6 model milestones** and removes the
**143-line core**, **7-line state adapter**, and unused **33-line stack helper**.
NMI and IRQ recognition, ordered acceptance effects, and mode-specific delivery
are chapter-owned. Shared named-entry support dispatches direct entry actions or
acknowledged modes, including supplied instructions through the same decoder.
Public bank aliases and nested snapshot views also come from the chapter.

Authored CPU TypeScript becomes **7,474 lines**, a net increase of **29**
for reusable entry and interface generation after deleting the Z80 adapters.
The chapter reaches **2,900 lines**, including **1,509 formal lines**,
and consolidates the API contracts, reference qualifications, and checks from
the former **808-line model document**. Total maintained CPU source is
**19,772 lines**; that measure now includes this model prose, previously
counted as separate documentation. The Z80 itself has zero handwritten
implementation, while shared runtime and compiler code remains TypeScript.

[Entry-language tests](../../tests/components/cpus/semantics/literate-interrupt-entries.test.ts)
generate a differently named CPU with different sources, choices, bank fields,
and bus width. Public Z80 chapter edits change entry gates, mode selection,
vectors, acceptance effects, and bank snapshots. Before/after comparisons match
...
The final 8088 slice completes **6 / 6 model milestones**, with **291 / 291
instruction forms** unchanged. The chapter now owns ordered INTR/NMI recognition,
wake-up effects, acknowledged/fixed vectors, the public interface, and integration
metadata. A shared vector-offer service preserves validation and callback order;
the generic coprocessor recorder supplies TEST/ESC effects without CPU policy.

This removes the **141-line core**, **6-line state adapter**, and **10-line
definition adapter**. The former device recorder becomes shared infrastructure.
Authored CPU TypeScript changes from **6,848 to 6,824 lines**, including the
new contract validation and generation support. The chapter grows from **3,517
to 4,115 lines**, with **2,714 formal lines**, while its separate model
document is removed. Much of the chapter growth is relocated hardware and host
contract prose. Total maintained CPU source in this table changes from **22,663
to 23,253 lines**; the former model document was outside that count.
This completes model ownership rather than claiming an overall source reduction.

Independent CPU and machine tests retain complete records and failure oracles.
Chapter-edit checks change memory and state schemas, public names, gates, masks,
fixed vectors, and acceptance actions. A differently named segmented chapter
also generates its TEST/ESC connections, snapshots, and external sources without
a CPU-name special case. Before/after comparisons match **16,384 first-byte
cases**, **23,040 ModR/M cases**, **1,536 reset/interrupt cases**, **4,768 additional
string/device cases**, and **17,003 injected failures**, including snapshots
inside callbacks.

The first 68000 foundation batch expands its chapter from **154 to 614
lines**, with **317 formal lines**, and raises literate instruction coverage
from **192 to 928 documented forms (2.6%)**. The complete stored schema earns
**1 / 6 model milestones**. The register recipes and handwritten status layouts
are removed; the state module is now a **6-line re-export/type adapter**.
Definitions shrink from **638 to 564 lines**. The native core is **607
lines**, including bindings to generated state readers.

Shared support for named state groups and conditional values brings authored
CPU TypeScript from **6,824 to 6,819 lines**. Total maintained CPU source is
**23,702 lines**, including state/inspection prose relocated from the
model document. This batch establishes chapter ownership and the needed language
constructs; it does not claim an overall source reduction. Before/after checks
match **131,072 instruction cases**, **256 reset/interrupt cases**, and **7,712
...
The final 68000 instruction migration adds **1,867 documented forms**, reaching
**36,029 / 36,029 (100%)** and **2 / 6 model milestones**. Control flow, MOVEP/MOVEM,
and status/system instructions retain **689 shared bodies**. Chapter-defined
encodings replace their three TypeScript catalogues. The remaining definition
adapter shrinks from **343 to 22 lines** and only groups chapter families.
MOVEQ also binds its opcode entries directly from the chapter. The unused
operand catalogue and Motorola condition helper are removed too.

The chapter is **7,366 lines**, including **4,537 formal lines**. Authored CPU
TypeScript falls from **6,473 to 6,003 lines**, a net reduction of **470**,
including the new cursor/target/reset syntax. The native core is **611 lines**.
Total maintained CPU source is **29,650 lines**, up from **28,541**, because
chapter prose and formal definitions add more lines than the removed TypeScript.
Generated output remains separate in the table above.

MOVEM uses a bounded sixteen-step traversal of mask, register index, and address,
with explicit per-register bank selection and a final captured-base commit.
Control families retain condition captures, separate cursor/target effects,
stack-before-target validation, and delayed pointer/counter commits. Status
families retain privilege checks, SR-before-update ordering, return-frame reads,
and the device reset signal. The native execution and exception boundary remains.

Independent probes retain every encoding and failed effect, while chapter edits
change conditions, cursor placement, byte spacing, mask order/count, sign
extension, privilege and commit order, return frames, and RESET encoding/signal.
A separate encoding edit changes the generated MOVEQ table.
The saved-build comparison matches **131,072 instruction cases**, **256
reset/interrupt cases**, and **10,980 injected failures**. Editor checks parse
all **889 CPU fences** and check keyword-like capture names alongside the new
boundary syntax. The complete clean-build regression passes **2,862 tests**;
the Zed extension also builds successfully, and all **1,857 local documentation
links** resolve.

The effective-address migration raises the 68000 to **3 / 6 model milestones**.
Its chapter now owns every register view/write rule, mode and brief-index
selection, signed extension, PC-relative base, A7 byte stride, and staged
address update. The native decoder, register-name arrays, and unused long-fetch
callback are removed. Native core size falls from **611 to 557 lines**;
authored CPU TypeScript falls from **6,003 to 5,994**, a net reduction of **9**
after the shared staging helper and checked language support are included.

The chapter has **7,549 lines**, including **4,635 formal lines**. Total
maintained CPU source is **29,824 lines**, up from **29,650**; the new
explanations and executable rules account for that growth. The separate model
document links to the chapter's address contract instead of repeating it.

Independent decoder probes cover every mode, live callbacks, both stack banks,
and failure at every observed effect. They exhaust **65,536 brief extensions**
for both An- and PC-relative addressing in each bank. Chapter edits change bank
selection, A7 stepping, signedness, PC capture order, and staging behavior. A
second CPU exercises the generic staging vocabulary, nested fields, lazy reads,
commit order, and capability restrictions. Saved-build comparisons match
**131,072 instruction cases**, **256 reset/interrupt cases**, and **12,748
injected failures**. Reset, execution, and exception delivery remain native.
...
The normal-execution migration raises the 68000 to **5 / 6 model milestones**.
Its [chapter contract](../../src/components/cpus/specifications/68000.md#fetching-dispatch-and-retirement)
owns whole-word fetch commits, logical cursor wrapping, initial alignment,
encoded input fields, IR/entry-phase updates, unknown-word selection, exception
vectors and saved-PC rules, retirement, STOP, and trace scheduling. Generated
bindings connect those rules to a shared word executor. The native core retains
exception-frame delivery, interrupt offers, and bus-failure classification.

The native core shrinks from **543 to 387 lines**. Shared word execution,
validation, and bindings bring authored CPU TypeScript from **6,064 to 6,243**
(**179 more lines**). The chapter has **7,742 lines**, including
**4,711 formal lines**. Total maintained CPU source is **30,270
lines**, up from **29,970**. This milestone transfers CPU policy into the chapter;
it does not claim an overall source reduction.

An unrelated word model exercises different state names, little-endian fetching,
and unaligned instruction addresses. Chapter edits change field extraction, IR
commits, retirement, trace sampling, vector literals, completion, unknown words,
and byte order through the public CPU. Runtime tests check every partial word,
boundary priority, cursor/target separation, modeled failures, and host throws.
Saved-build comparisons match **196,608 instruction cases** (every word in three
state configurations), **256 reset/interrupt cases**, and **12,748 injected failures**.
The full clean-build regression passes **2,890 tests**. Zed parses all **896 CPU
fences**, the extension builds, and all **1,892 local documentation links** resolve.

The external-event migration completes the 68000's **6 / 6 model milestones**,
bringing all eight models to that point. The
[chapter](../../src/components/cpus/specifications/68000.md#exception-frames-and-external-events)
authors short and memory-error frames, their transfer order, supervisor/status
changes, saved-PC selection, interrupt recognition and acknowledgement, initial
handler-fetch recovery, and terminal halt after a second modeled fault.
A shared entry sequence binds these actions and read-only decision sources;
there are no native 68000 frame constructors or interrupt acceptance rules.

The native core shrinks from **387 to 237 lines**. It retains public types,
snapshots, guards, connection validation, and physical-memory recording. Shared
entry machinery and language validation bring authored CPU TypeScript from
**6,243 to 6,386 lines** (**143 more**). The chapter reaches **7,946 lines**,
including **4,831 formal lines**; maintained CPU source totals **30,619 lines**,
up from **30,270**. At that point public-interface generation and adapter removal
were still needed; the following migration completes them.

Chapter mutation tests alter frame reservation and transfer addresses, vector
geometry, interrupt gates and callback-visible masks, initial-fetch recovery,
and special-status bits through the public CPU. Shared runtime tests use different
frame sizes, vector numbers, level ranges, and reason names, checking partial
entry, nested recovery, and unchanged host throws. Saved-build comparisons match
**196,608 instruction cases**, **256 reset/interrupt cases**, and **12,748 injected
failures**. An additional boundary matrix matches **662 cases**, **8,300 failed
transfers/host throws**, and **9,880 second faults**, comparing records, memory,
and snapshots observed inside callbacks. The full clean-build regression exercised
**2,899 tests**: **2,898 passed**, with only the old expected definition count
failing. After updating it to **10,111**, all **28 report tests** pass on rerun;
the other **2,871 tests** passed in the full run. Zed parses **901 CPU fences**,
the extension builds, and all **1,901 local documentation links** resolve.

The final 68000 public-interface migration completes **zero CPU-specific
handwritten implementation across all eight models**. Its chapter generates
class and record types, detached snapshot views, bus geometry, and machine
metadata. The **237-line core**, **25-line context adapter**, **six-line state
adapter**, and **22-line definition adapter** are removed, together with their
manual registry entries. Word modules group automatically by declared inputs;
explanations retain chapter-family order independently of that partitioning.
`boundary word` selects validator capabilities without a processor-name branch.

Authored CPU TypeScript falls from **6,386 to 6,357 lines** (**29 fewer**).
The chapter grows to **8,313 lines**, including **4,836 formal lines**,
while taking ownership of the former **1,015-line model document**, hardware
references, and background. Maintained CPU source under the table's definition
is **30,965 lines**, up from **30,619**; including the removed model document
in both sides gives a net reduction of **669 lines**.

Public-interface tests rename the CPU and class, change physical bus width and
snapshot views, and reject missing reset/event contracts. Shared memory checks
use other address widths, including a full unsigned 32-bit bus, and distinguish
explicit bus failures from host throws. Saved-build comparisons preserve
**196,608 instruction cases**, **256 reset/interrupt cases**, **12,748 injected
failures**, **662 boundary cases**, **8,300 transfer/host failures**, and **9,880
second faults**, including snapshots observed during callbacks.

The final clean-build regression passes **all 2,904 tests**, with no skips.
...
reader is also now generated only by its chapter test. Production state readers
provide the chapters' declared register/status views; the 68000 also exposes its
chapter-owned effective-address decoder. One generated indexed decoder
is shared by 6809 base-page and prefixed bodies. Readers earn no separate opcode
coverage credit.
...
[Literate specification and model contract](../../src/components/cpus/specifications/8008.md) ·
...
[Executable specification and model contract](../../src/components/cpus/specifications/8080.md) ·
...
[Executable specification and model contract](../../src/components/cpus/specifications/6800.md) ·
...
waiting state and generated factory types. See the [model contract](../../src/components/cpus/specifications/6800.md#waiting-and-external-interrupt-delivery)
for current-mask recognition, snapshot, and host-failure policies.

## 6502

[Executable specification and model contract](../../src/components/cpus/specifications/6502.md) ·
...
[model contract](../../src/components/cpus/specifications/6502.md#status-as-a-byte) documents those supplementary checks.
...
the [arithmetic contract](../../src/components/cpus/specifications/6502.md#arithmetic-in-binary-and-decimal) describes
...
[Literate chapter and model contract](../../src/components/cpus/specifications/6809.md) ·
...
The [reset preservation policy](../../src/components/cpus/specifications/6809.md#reset-and-external-entry) does not claim
hardware power-on values for unspecified state. The [delivery contract](../../src/components/cpus/specifications/6809.md#execution-and-public-interface)
...
[Literate chapter and model contract](../../src/components/cpus/specifications/z80.md) ·
...
those cycles. See the [model contract](../../src/components/cpus/specifications/z80.md) for unsupported-attempt and
...
[arithmetic contract](../../src/components/cpus/specifications/z80.md#accumulator-arithmetic-and-logic) documents those checks.
...
see [checks and limits](../../src/components/cpus/specifications/z80.md#checks-and-limits).
...
at every step. See [checks and limits](../../src/components/cpus/specifications/z80.md#checks-and-limits).
...
all 24 I/O forms; see [checks and limits](../../src/components/cpus/specifications/z80.md#checks-and-limits).
...
the final seven forms passed; [checks and limits](../../src/components/cpus/specifications/z80.md#checks-and-limits)
distinguish those vectors from delivery and timing verification.

## 8088

[Literate specification and model contract](../../src/components/cpus/specifications/8088.md) ·
...
[Source](../../src/components/cpus/specifications/68000.md) ·
...
| Alignment | Odd instruction/word/long accesses enter vector 3 with a seven-word frame; byte operands remain valid; [staged operand and terminal-halt policy](../../src/components/cpus/specifications/68000.md#address-errors) |
| Bus errors | Explicit failed bytes enter vector 2; shared extended frame, partial effects, first-handler-fetch context, and terminal halt on a second fault; [recovery contract](../../src/components/cpus/specifications/68000.md#bus-errors) |
...
| Exceptions | Six-byte supervisor frames, synchronous and trace vectors, all illegal and line-A/line-F encodings, external interrupt acknowledgement, saved PC/SR, and RTE; [entry and failure contract](../../src/components/cpus/specifications/68000.md#synchronous-exception-entry-and-return) |
| External controls | [Selected level offers](../../src/components/cpus/specifications/68000.md#external-interrupt-delivery), all vector bytes, autovectors/spurious response, trace priority, STOP wakeup, and [RESET callback](../../src/components/cpus/specifications/68000.md#reset-device-connection) |
